...
def wise_speak(sentence):
    ...
    sentence_core = sentence[:-1] if punctuation else sentence
    sentence_array = sentence_core.split(' ')

    for i in range(len(sentence_array)):
        word = sentence_array[i].lower().strip('.,!?;:')
        if word in words:
            first_part = sentence_array[:i+1]
            last_part = sentence_array[i+1:]
            first_part = [w.lower() for w in first_part]
            if last_part:
                last_part[0] = last_part[0].capitalize()
                result = ' '.join(last_part) + ', ' + ' '.join(first_part) + punctuation
            else:
                result = ', '.join([first_part[0].capitalize()] + first_part[1:]) + punctuation
            return result
...
print(wise_speak("They will succeed."))
print(wise_speak("We must learn."))
